...
function createCliFixtureWorkspace({
  ...
  if (exitCode !== undefined && exitCode !== 0) {
    fs.writeFileSync(
      path.join(fixtureDir, "exit-code.txt"),
      `${exitCode}`,
      "utf8",
    );
  }
...
function createAssertionDirectory({
  ...
  if (exitCode !== undefined) {
    fs.writeFileSync(
      path.join(directoryPath, "exit-code.txt"),
      exitCode,
      "utf8",
    );
  }
...
test("main prints one numbered failure line for each failed fixture after the summary output", async () => {
  const {
    workspaceRoot,
    testsDir,
    fixtureDir: firstFixtureDir,
  } = createCliFixtureWorkspace({
  ...
  fs.writeFileSync(
    path.join(secondFixtureDir, "stdin.txt"),
    "openspec-diff\nhello\\n\n",
    "utf8",
  );
...
test("assertFixtureResult returns exitCode 0 when expected and actual fixture directories match", () => {
  ...
  try {
    assert.deepEqual(assertFixtureResult({ expectedPath, actualPath }), {
      exitCode: 0,
    });
  } finally {
