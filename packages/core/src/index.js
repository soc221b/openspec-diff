...
const CHANGE_ROOT_PARTS = ['openspec', 'changes'];
export async function findChanges(repoRoot) {
    const changesDir = path.join(repoRoot, ...CHANGE_ROOT_PARTS);
    const entries = await fs.readdir(changesDir, { withFileTypes: true }).catch((error) => {
        if (error.code === 'ENOENT') {
            return [];
        }
        throw error;
    });
    const changes = await Promise.all(entries
...
export function isChangeMarkdownFile(repoRoot, filePath) {
...
export function inferChangeNameFromPath(repoRoot, filePath) {
...
export async function generateChangeDiff(repoRoot, changeName) {
    ...
    const files = await Promise.all(changeFiles.map(async (absolutePath) => {
        ...
        const previousPath = previousAbsolutePath && previousContent !== ''
...
export function formatChangeDiff(result) {
...
function resolvePreviousPath(repoRoot, changePath, relativePath) {
...
function buildUnifiedDiff(previousPath, currentPath, previousContent, currentContent) {
