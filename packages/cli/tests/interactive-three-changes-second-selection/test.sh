...
TMP_DIR=$(mktemp -d)
CAPTURE_TIMEOUT_SEC=3
PROMPT_CAPTURE_DELAY_SEC=1
...
(
	cd "$TMP_DIR"
	timeout "$CAPTURE_TIMEOUT_SEC" sh -c '
		cli_bin=$1
		prompt_capture_delay_sec=$2
		{ printf "\033[B"; sleep "$prompt_capture_delay_sec"; tail -f /dev/null; } | "$cli_bin"
	' sh "$CLI_BIN" "$PROMPT_CAPTURE_DELAY_SEC"
