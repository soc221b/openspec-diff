# Transaction - Changes

## MODIFIED Requirements

### Requirement: The system SHALL record each completed purchase
The system SHALL record each completed purchase with the linked order tracking summary.

#### Scenario: A customer reviews a completed purchase
- **WHEN** a completed purchase is opened from order history
- **THEN** the purchase details are shown with the linked tracking summary
